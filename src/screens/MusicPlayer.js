...
const MusicPlayer = ({ navigation, route }) => {
  ...
  const id = "64e3fa37-2666-4280-9a2e-3d01d8f99604";
  ...
  const updateList = async () => {
    try {
      return await API.get(`${BASE_URL}/musicas`)
  ...
  const callPlayMidiByName = () => {
    ...
    let play = "playMidiByName(" + arrayList.urlArquivo + "); true;";
  ...
  const getLesson = async () => {
    console.log("Id: ", id);
    try {
      return await API.get(`${BASE_URL}/musicas/${id}`)
        .then((res) => {
  ...
  return (
    ...
    >
      ...
      <WebView
        ref={webViewRef}
        source={{
          uri: "http://atendimento.caed.ufmg.br:8000/ebooker/magickeys/embedplayer.php",
        }}
        onMessage={onMessage}
