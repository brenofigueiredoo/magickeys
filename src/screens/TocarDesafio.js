...
const rightNote = "#008181";
const wrongNote= "#ff0060";
const defaultNote = red;


const TocarDesafio = ({ navigation, route }) => {

  const { user, getProfile } = useContext(UserContext);

  //console.log("User id: ", user);
  const [progressoDesafio, setProgresssoDesafio] = useState(0);

  const { id } = route.params;
  //console.log("Id música", id);
  ...
  const [notesPlayed, setNotes] = useState(0);
  ...
  const handleTouch = (event, play = true) => {
    ...
    setNotes(notesPlayed+1);
    
    if(clickedShape.noteName===expectedNote) {
      setProgresssoDesafio(progressoDesafio+1);
      
      setKeyColor(rightNote);

    }else{

      setKeyColor(wrongNote);

    }
    console.log("Progresso desafio: ", progressoDesafio,",",notesPlayed, "length",endOfFile);

    if(notesPlayed===endOfFile){
      console.log("create desafios");
      createDesafio();

    }
    else{
      console.log("Create desafios not called");
    }
  ...
  const createChallengeProgress = async (data) => {
    try {
      const response = await API.post(`${BASE_URL}/progresso-desafios`, data);
  ...
  const getProgress = async () => {
    try {
      const response = await API.get(`${BASE_URL}/progresso-licoes/licao/${id}`);
  ...
  const updateProgress = async (data) => {
    ...
    try {
      const response = await API.put(`${BASE_URL}/progresso_licoes/${progressId}`, data);
  ...
  const createDesafio=()=>{
    ...
    var pontuacao = progressoDesafio/endOfFile * 100;

      console.log("Pontuacao: ", pontuacao.toFixed(2)),
      console.log("Número de notas: ", endOfFile);
      var data = {
          desafioId: id,
      ...
      createChallengeProgress(data);



  }
  const [endOfFile, setEndOfFile] = useState();
  const onMessage = (event) => {
    ...
    if(msg.type==="EOC"){
      ...
      console.log("EOC type message called: ", msg.type, " ", msg.endOfChallenge)
  ...
  const getDesafio = async () => {
    ...
    try {
      return await API.get(`${BASE_URL}/desafios/${id}`)
        .then((res) => {
  ...
  return (
    ...
    >
      ...
      <WebView
        ref={webViewRef}
        source={{
          uri: "http://atendimento.caed.ufmg.br:8000/ebooker/magickeys/embedplayerchallenge.php",
        }}
        onMessage={onMessage}
