...
progressoDesafiosRouter.get(
  "/progresso-desafios/desafio/:desafioId",
  authMiddleware,
  (req, res) => {
    try {
      ...
      if (!desafio) {
        return res.status(404).json({ error: "Desafio não encontrada" });
      }

      // Buscar o progresso do usuário para este desafio
      const progresso = router.db
        .get("progresso_desafios")
        .find({ usuarioId, desafioId })
        .value();
      ...
      if (!progresso) {
        return res.status(404).json({
          ...
          message: "O usuário ainda não iniciou este desafio",
        });
      }
      ...
      return res.status(200).json({
        ...
        dataProgresso: progresso.dataProgresso,
      });
    } catch (error) {
      return res
        .status(500)
        .json({ error: "Erro ao buscar progresso do desafio" });
    }
  }
);
